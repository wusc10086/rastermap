...
L.DivHeatmapLayer = L.FeatureGroup.extend({
  ...
  _addBlob: function(lat,lng,value,old_marker){
    ...
    opacity = value;
     ...
     if(value>=0.8)
     {
       gradient =  'radial-gradient(closest-side, rgba(178,34,34'+', '+alpha_start+') 0%, rgba(178,34,34'+', '+alpha_end+') 100%)';
     }
      else if(value>=0.6&&value<0.8)
     {
       gradient =  'radial-gradient(closest-side, rgba(255,255,0'+', '+alpha_start+') 0%, rgba(255,255,0'+', '+alpha_end+') 100%)';
     }else
     {
         //alpha_start 暂时替换成0。5
        gradient =  'radial-gradient(closest-side, rgba('+ this._parseColor(this.options.color) +', '+alpha_start+') 0%, rgba('+ this._parseColor(this.options.color) +', '+alpha_end+') 100%)'; 
     }
