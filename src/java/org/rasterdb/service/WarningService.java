...
//    String condition= "select t1.Num,t1.Latitude,t4.Status,t1.Longitude,t2.AlertArede,t2.curdata,t4.average from efdevice t1"
// +" left join efdata_warning t2 on t2.deviceid=t1.Num"
//+" left join (select s1.deviceid,s1.average,s1.status from efdata_today s1 , (select deviceid,max(SendTime)  as MaxDate from efdata_today group by deviceid) s2 "
//+" where s1.SendTime=s2.MaxDate and s1.deviceid=s2.deviceid) t4 on t4.deviceid=t1.Num";
        String condition = "select t1.Num,t1.Latitude,t1.Longitude,t4.sendtime,t4.average from efdevice t1\n"
                + "left join (select s1.deviceid,s1.average,s1.sendtime from efdata_today s1 , (select deviceid,max(SendTime)  as MaxDate from efdata_today group by deviceid) s2 \n"
                + " where s1.SendTime=s2.MaxDate and s1.deviceid=s2.deviceid) t4 on t4.deviceid=t1.Num";
