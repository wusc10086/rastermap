...
public class DeviceManagerModule {
    ...
    private void deviceListConvert(ViewModel model) {
        List<ProjectInfo.XCity> cities = (List<ProjectInfo.XCity>)model.get("cities");
        Map<String,String> cityidAndName = new HashMap<String,String>(100);
        for (ProjectInfo.XCity city :cities) {
            cityidAndName.put(city.getCityid(), city.getName());
        }
        
        List<Device> deviceList = new ArrayList<Device>();
        
        List<EFDevice> efdeviceList = (List<EFDevice>)model.get("deviceList");
        for (EFDevice efdevice : efdeviceList) {
            Device device = Device.toDevice(efdevice);
            ...
            String ciyName = cityidAndName.get(cityid);
            device.setCityName(ciyName == null ? cityid : ciyName);
        }
        model.put("deviceList", deviceList);
    }
